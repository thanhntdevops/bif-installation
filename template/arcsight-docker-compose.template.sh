...
services:
  ...
  ${TENANT}-fetch-alerts:
    image: ${SERVICE_IMAGES[arcsight-integration]}
...
#      - redis
  ...
  threatconnect-worker:
    image: ${SERVICE_IMAGES[threatconnect-worker]}
    container_name: threatconnect-worker
    restart: unless-stopped
    volumes:
      - ./.env.threatconnectworker:/app/.env:ro
      - ./logs:/app/logs:rw
    user: 1000:1000
    networks:
      - bif_network
